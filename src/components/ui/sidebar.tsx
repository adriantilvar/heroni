...
function SidebarProvider({
  ...
  const setIsOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: Temporary solution-should explore another one
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    [state, isOpen, setIsOpen, isMobile, isOpenMobile, toggleSidebar]
  );
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-sidebar-border border-l px-2",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
